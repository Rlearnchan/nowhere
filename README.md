...
# news-linked featured stocks plus KRX sector temperature. Tickers come from jibi and optional explicit codes.
python -m nowhere.cli brief collect-fixtures --run-id collect-featured-sector \
  --jibi fixtures/jibi/sample_events.json \
  --live-krx --live-featured-stocks --featured-stock-ticker 005930
...
공개 HTML/PDF 상단에는 내부 QA 용어 대신 독자가 읽을 수 있는 상태 카드가 표시됩니다. 한국 시장은 장중/최신 Naver 지수 스냅샷을 대표값으로 우선 표시할 수 있고, KRX는 공식 종가 및 업종 세부 근거로 함께 남깁니다. 글로벌 맥락은 Yahoo Finance 최신성, 뉴스 근거는 시장 연결 보강 필요 여부, 브리프 상태는 초안/게시 가능 여부를 나눠 보여줍니다. 특징주 섹션은 jibi 뉴스 tickers 또는 `--featured-stock-ticker`에서 고른 종목의 네이버 스냅샷을 표시하고, 업종 온도계는 KRX 세부 지수 행에서 상승/하락 상하위를 추출합니다.
